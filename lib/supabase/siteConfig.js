  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message?.includes('does not exist') || error.message?.includes('relation')) {
        // Table doesn't exist - this is okay, return null
        console.warn('site_config table does not exist. Run migration_site_config.sql in Supabase.')
        return null
      }
      
      if (error.code === 'PGRST116') {
        // No rows returned - this is okay, return null
        return null
      }
    ...
    if (error?.message && error.message.trim()) {
      console.warn('Error in getSiteConfig:', error.message)
    }
    return null
  }
}
