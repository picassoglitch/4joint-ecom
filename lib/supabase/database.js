...
export async function createProduct(productData, userId = null) {
  ...
  // Include quantity and unit if provided
  if (productData.quantity !== undefined && productData.quantity !== null) {
    productPayload.quantity = productData.quantity
  }
  if (productData.unit) {
    productPayload.unit = productData.unit
  }
...
export async function updateVendor(vendorData) {
  ...
  if (!session?.user) {
    throw new Error('Debes iniciar sesión para actualizar la información de la tienda. Por favor, inicia sesión e intenta de nuevo.')
  }

  // Separate new columns that might not exist yet
  const {
    service_colonias,
    show_store_location,
  ...
  if (error && (error.message?.includes('service_colonias') || 
                error.message?.includes('show_store_location') || 
                error.message?.includes('show_whatsapp_contact') || 
                error.message?.includes('whatsapp_number') ||
                error.code === '42703' || 
                error.hint?.includes('service_colonias') ||
                error.hint?.includes('show_store_location'))) {
    console.warn('⚠️ Some columns may not exist yet. Updating with standard fields only. Run migration: supabase/migration_colonias_and_courier.sql')
    ...
    if (retryResult.error) {
      const errorMessage = retryResult.error.message || retryResult.error.details || 'Error al actualizar la información de la tienda'
      const enhancedError = new Error(errorMessage)
      enhancedError.code = retryResult.error.code
      enhancedError.details = retryResult.error.details
      enhancedError.hint = retryResult.error.hint
      console.error('Supabase error updating vendor:', retryResult.error)
      throw enhancedError
    }
    
    console.log('✅ Tienda actualizada (sin campos nuevos). Para habilitar todas las funciones, ejecuta la migración en Supabase.')
    return retryResult.data
  }

  if (error) {
    const errorMessage = error.message || error.details || 'Error al actualizar la información de la tienda'
    const enhancedError = new Error(errorMessage)
    enhancedError.code = error.code
    enhancedError.details = error.details
    enhancedError.hint = error.hint
