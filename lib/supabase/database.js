...
export async function updateVendor(vendorData) {
  ...
  // Filter out undefined values and fields that might not exist in the database
  ...
  if (error) {
    ...
    console.error('Data being updated:', cleanedData)
    throw new Error(error.message || 'Error al actualizar la información de la tienda')
  }
  return data
}
 ...
 * Used by webhook to update order status after payment
 */
export async function updateOrderPayment(orderId, paymentData) {
  ...
  // Map payment status to our order status (supports PayPal and other providers)
  const statusMap = {
    // PayPal statuses
    ...
    'VOIDED': 'ORDER_FAILED',
    'COMPLETED': 'ORDER_PAID',
    // Legacy Mercado Pago statuses (for backward compatibility)
    'approved': 'ORDER_PAID',
    'pending': 'ORDER_PENDING',
    'in_process': 'ORDER_PENDING',
    'rejected': 'ORDER_FAILED',
    'cancelled': 'ORDER_FAILED',
    'refunded': 'ORDER_FAILED',
    'charged_back': 'ORDER_FAILED',
  }

  // Determine payment provider
  const paymentProvider = paymentData.payment_provider || 'PAYPAL'
  
  // Determine if payment is completed/approved
  const isPaid = paymentData.is_paid !== undefined 
    ? paymentData.is_paid 
    : (paymentData.status === 'COMPLETED' || paymentData.status === 'approved' || paymentData.status === 'APPROVED')

  // Map status first (before creating payload)
  let mappedStatus = 'ORDER_PENDING'
  if (paymentData.status) {
    if (paymentData.status.startsWith('ORDER_')) {
      // Already mapped status (e.g., ORDER_PAID, ORDER_PENDING)
      mappedStatus = paymentData.status
    } else {
      // Map from payment provider status (e.g., 'approved' -> 'ORDER_PAID')
      mappedStatus = statusMap[paymentData.status] || 'ORDER_PENDING'
    }
  }

  const updatePayload = {
    payment_id: paymentData.paymentId || paymentData.payment_id || paymentData.captureId,
  ...
  // Only add payment_status if column exists (safe to include, will be ignored if column doesn't exist)
  if (paymentData.status) {
    updatePayload.payment_status = paymentData.status
  }

  // Only add paid_at if payment is completed/approved and column exists
  if (isPaid) {
    const dateApproved = paymentData.dateApproved || paymentData.date_approved || paymentData.paid_at
    if (dateApproved) {
      updatePayload.paid_at = dateApproved
    } else {
      // If no date provided but payment is approved, use current timestamp
...
export async function createOrder(orderData) {
  ...
  if (error) {
    ...
    if (error.message?.includes('cod_payment_preference')) {
      missingColumns.push('cod payment preference')
    }
