...
if (typeof window !== 'undefined' && !supabase) {
  console.error('❌ Supabase client is not initialized in auth.js')
}
  ...
  try {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        // SECURITY: Force email confirmation - account is inactive until verified
        // Do NOT set autoConfirm: true - this would bypass email verification
      },
    })
...
export async function signIn(email, password) {
  try {
    ...
    if (!supabase) {
      ...
      return {
        data: null,
        error: { message: 'Error de configuración. Por favor recarga la página.' }
      }
    }
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      ...
      return {
        data: null,
        error: { message: 'Error de configuración. Por favor contacta al administrador.' }
      }
    }
...
export async function signInWithGoogle() {
  try {
    ...
    // signInWithOAuth returns a URL that should be used to redirect
    // The redirect happens automatically, but we return the data for error handling
    return { data, error: null }
  } catch (error) {
