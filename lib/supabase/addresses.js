...
export async function saveAddress(addressData) {
  ...
  // Silently return null if no session instead of throwing error
  // This prevents error messages from showing to users
  if (!session?.user) {
    console.warn('No user session available. Cannot save address.');
    return null
  }

  // Ensure user_id matches the authenticated user (required for RLS)
  const addressPayload = {
    user_id: session.user.id, // This MUST match auth.uid() for RLS to work
  ...
  if (!addressPayload.user_id || typeof addressPayload.user_id !== 'string') {
