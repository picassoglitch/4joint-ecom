...
/**
 * Get best available address object from order/orderData
 * Supports guest checkout JSON and addresses(*) join.
 */
function getOrderAddress(orderLike) {
  if (!orderLike) return null
  if (orderLike.guest_address) return orderLike.guest_address
  // Some queries may return address as array; normalize to object
  if (Array.isArray(orderLike.address)) return orderLike.address[0] || null
  if (orderLike.address) return orderLike.address
  return null
}

/**
 * Human-friendly delivery option SLA text
 */
function getDeliveryOptionDisplay(deliveryOptionId) {
  if (!deliveryOptionId) return null
  switch (deliveryOptionId) {
    case 'same_day':
      return 'Entrega Mismo Día (antes de 8pm)'
    case 'on_demand':
      return 'On Demand (60-80 min)'
    default:
      return String(deliveryOptionId)
  }
}

function getFulfillmentDisplay(order) {
  const fulfillmentType = order?.fulfillment_type || null
  const deliveryOptionId = order?.delivery_option || null

  // If it's delivery, include delivery option + SLA
  if (fulfillmentType === 'delivery') {
    const opt = getDeliveryOptionDisplay(deliveryOptionId)
    return opt ? `Envío a domicilio - ${opt}` : 'Envío a domicilio'
  }

  if (fulfillmentType === 'pickup') return 'Pickup (recoger en tienda)'
  if (fulfillmentType === 'meetupPoint') return 'Punto de entrega'
  if (fulfillmentType === 'courierExterno') return 'Envío coordinado (courier externo)'

  // Fallback: if no fulfillment_type but delivery_option exists, assume delivery
  if (!fulfillmentType && deliveryOptionId) {
    const opt = getDeliveryOptionDisplay(deliveryOptionId)
    return opt ? `Envío a domicilio - ${opt}` : 'Envío a domicilio'
  }

  return fulfillmentType || deliveryOptionId || 'N/A'
}
    ...
    if (notificationType === 'newOrder' && orderId) {
      ...
      if (addr) {
        const addressParts = [
          ...
          addr.state,
          addr.zip,
          // Optional references help drivers
          addr.references,
        ].filter(Boolean)
...
export function formatOrderNotification(order, orderItems) {
  ...
  const codPaymentPreference = order.cod_payment_preference || null
  ...
  // COD payment preference (cash vs terminal)
  if (paymentMethod === 'COD' && codPaymentPreference) {
    const codDisplay = codPaymentPreference === 'terminal'
      ? 'Tarjeta \\(llevar terminal\\)'
      : 'Efectivo'
    // Match screenshot wording
    message += `💵 *Cobro:* ${escapeMarkdownV2(codDisplay)}\n`
  }
  
  // Delivery / fulfillment (with delivery option SLA)
  const fulfillmentDisplay = escapeMarkdownV2(getFulfillmentDisplay(order))
  message += `🚚 *Entrega:* ${fulfillmentDisplay}\n\n`
  ...
  // Address (guest_address or addresses join)
  ...
  if (addr) {
    ...
    const references = escapeMarkdownV2(addr.references || '')
...
/**
 * Create inline keyboard for order actions (only Maps button)
 */
