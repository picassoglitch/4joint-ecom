...
export function formatOrderNotification(order, orderItems) {
  ...
  const itemsList = orderItems
    .map(item => {
      ...
      return `• ${productName}${variant} ×${quantity} \\- $${total.toFixed(2)}`
    })
    .join('\n')
  ...
  if (order.guest_address) {
    ...
    if (city || state || zip) {
      ...
      if (addressLine) message += `${addressLine}\n`
    }
    message += `\n`
  }
  ...
  if (order.tip_amount && parseFloat(order.tip_amount) > 0) {
    message += `💵 *Propina:* $${parseFloat(order.tip_amount).toFixed(2)} MXN\n\n`
  }
